...
The gap between those two artifacts is most of the engineering. This post is about how we closed that gap at Aurelian.
...
Our budget for a fluid conversation was around 600ms p95. Above 800ms it starts to feel like the agent is thinking. Above 1.2s the citizen will start to repeat themselves. Above 2s they'll assume the line dropped.
...
Every milestone in the project came from finding a place where we could overlap something instead of waiting for it.
...
**1. STT runs continuously.** We don't wait for the user to "finish" before transcribing. We keep a rolling transcript with confidence scores. Interim results feed downstream context. By the time the endpointer decides the user is done, we already have the final transcript ready.

**2. LLM starts before STT finalizes.** The moment the endpointer says "probably done" with high confidence, we kick off the LLM with the current rolling transcript. If the user keeps talking after, we cancel and restart. Most of the time they don't, and we've saved 200-400ms.
...
The cost of all this overlap is cancellation. Every layer has to support cancellation cleanly, because we cancel a lot. STT might decide on a re-segmentation. The LLM might get interrupted by the user starting a new turn. TTS might need to abort mid-utterance and switch to a new response. Cancellation paths are *first-class*, not exception cases.
...
Real callers don't speak in clean sentences. They pause to think. They use filler words. They start a sentence, abandon it, restart. We needed an endpointer that understood semantics, not just silence.

We layered three signals:
...
What's tricky: detecting that the new audio is the citizen interrupting and not the agent's own audio leaking through into the input channel. Echo cancellation isn't perfect. We had to combine:
...
User says: "I want to report a [pause] cat in a tree." The interim transcript at 400ms is "I want to report a." If you start the LLM there, it'll produce a confidently wrong response — "Okay, what would you like to report?" — that the user will then have to correct.
...
We ended up with a class of agent responses we called *acknowledgements*: cheap, generic, polite phrases the agent could emit while waiting for more signal. They bought us latency budget without making the conversation feel robotic.
...
We tuned chunk size per response type. Greetings and acknowledgements stream small (low-context, fixed prosody anyway). Substantive responses buffer up a sentence's worth of tokens before the first TTS frame goes out — sacrificing some latency to keep the voice human.
...
We wrote a test harness that fuzzed cancellation timing — random aborts at every layer — and ran it as part of CI. It caught a class of bugs that no functional test would have surfaced.

## Trade-offs that mattered

**Smaller chunks vs better prosody.** We compromised: cheap responses stream small, substantive responses buffer a sentence. Different voices for different jobs.

**Lower-latency model vs higher-quality model.** We used a faster model for trivial turns (acknowledgements, intent gathering) and a stronger model for high-stakes turns (incident description, escalation reasoning). A small router decided which model to fire per turn, in code, based on the conversation state. The router itself rode on the [confidence-aware policy layer](/blog/confidence-aware-routing) — same primitive, different decision.

**Cached responses vs always-live generation.** Greetings, holds, and "let me get a dispatcher" responses were all pre-rendered audio, played from cache. The cache was per-agency (each agency has its own preferred phrasing). For everything else we generated live. Caching where you can buys back latency budget for the cases where you can't.

**Transport vs synthesis.** We initially deployed in a single region. Latency to West Coast callers was fine; latency to East Coast callers was not. Multi-region voice deployment turned out to be more about audio transport latency than inference latency — we'd been optimizing the wrong number.
...
We shipped at sub-600ms p95 TTFAR for the typical conversational turn, and 250-400ms for cached or acknowledgement responses. The faster end of that range is fast enough that callers stop noticing the AI is an AI, which is — for our domain — exactly the bar.
