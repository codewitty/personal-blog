---
title: Why We Built Our Own LLM Eval Stack — and Why Retool Was the Best Decision We Made
description: Every "production-ready" LLM eval framework we tried wanted to own our prompts. None could grade whether a 911 response was correct. Here's what we built instead, and why a Retool front-end was the unlock that made the whole loop work.
...
When we started shipping conversational AI for non-emergency public-safety calls at [Aurelian](https://www.aurelian.com/), we had the same problem every team that ships LLMs in production has: we were iterating on prompts blindly. A change that "felt better" in the dev loop would silently regress correctness in prod. Latency would creep on a Wednesday and nobody could tell which of three prompt edits caused it. We had no way to A/B with eval coverage, no way to attribute regressions to a specific prompt version, and — worst — no way to involve the people who actually knew whether the AI's response was correct.
...
So when I started looking at the LLM observability and eval landscape — PromptLayer, LangSmith, Helicone, Braintrust, Humanloop, Promptfoo, Phoenix, Vellum — I was looking for something specific: a tool that could close the loop between subject-matter-expert judgment and the prompts running in production.

I couldn't find one.

This is the story of why, and what we ended up building instead.
...
We had every one of these. The first iteration of the prompt observability work wasn't a console — it was a stack of regret.

## Why we tried — and rejected — every off-the-shelf option

We *wanted* to buy. We tried hard. Here's where each tool we evaluated broke down for a high-stakes voice domain.
...
"Correctness" in our domain looks like:
...
Every eval framework we evaluated was built for ML engineers writing Python. The interaction model was: engineer writes a grader, runs it, looks at a dashboard. The dashboards were dashboards for engineers — token counts, latency histograms, judge-prompt diffs.
...
No tool we tried had a surface that was usable by a non-engineer without us writing the UI ourselves anyway. And once we were writing the UI ourselves, we were already most of the way to writing the framework.

### Voice and multi-turn were second-class citizens

Most frameworks assumed a single-turn text task. Ours was multi-turn audio with structured incident state attached. Stitching that into a generic eval frame meant fighting the framework on every column we cared about.

### Per-token latency wasn't first-class

For voice, what matters isn't end-to-end latency on a single call. It's time-to-first-token (TTFT) and per-utterance latency, broken down by prompt version, by intent, by jurisdiction. The frameworks measured end-to-end. We needed slices. We were going to build the slicing logic ourselves anyway.
...
This was a quieter dealbreaker but a real one. Most frameworks expect prompts to live in *their* registry, edited in *their* UI, deployed via *their* SDK. For a regulated domain we wanted prompts in the repo: code-reviewable, version-controlled with the application code, deployable atomically with the code that consumed them. Putting prompts behind a vendor's web app meant:
...
"Block deploy if eval regresses" wasn't a primitive in any framework we evaluated. We were going to write the CI integration ourselves regardless. If we were writing the integration glue *and* most of the slicing *and* most of the SME UI, we were already writing the framework.

The build-vs-buy math kept landing in the same place: integration work to make any of these tools fit our process was equal to or greater than building it from scratch. So we built it.
...
The registry generates a content hash at build time. Every deployed binary knows the hash of every prompt it's pinned to. Telemetry rows reference prompts by hash — not by name — so we can trace a regression all the way back to the literal bytes that produced it.

Code-reviewable, atomic, immutable in production. No vendor outage takes down our prompt store.

### 2. The eval runner

A queue-based service that runs prompt versions against a curated dataset of canonical transcripts.

We had two grader families:
...
We ran the full eval suite on every prompt change in CI, and on a rolling cadence in prod against a sampled stream of real calls — sanitized, tightly access-controlled.
...
A judge grader is a thin wrapper over a model call with a strict schema response, plus a confidence threshold below which we kick the case to a human.
...
Per content hash. So when we shipped intake v23 and p95 regressed by 200ms, we could see the regression by Friday afternoon and roll back by Friday evening — not by next Tuesday, after a customer ticket.

### 4. The Retool layer (the unlock)

This is the part I'm proudest of, and the part I think most teams overlook.

We built the entire SME-facing surface in Retool, pointed at our internal API. Five screens:
...
Building this UI from scratch in React would have eaten months of frontend engineering we did not have. Retool let us ship the SME surface in roughly the same time it took to build the backend that powered it.
...
**Invest in the canonical-transcript dataset earlier.** We backfilled it from real calls. The backfill was painful and the early eval suite was noisy until the dataset matured. Start with a curated 50-example seed dataset before you have a single prompt in production.

**Separate dev / staging / prod prompt pools from day one.** We had one prompt registry that served all three environments and had to evolve into separate pools. Should have been separate from the start.

**Treat the Retool app as a real product.** Versioning, change history, RBAC, on-call ownership. We eventually got there. The first six months it was "the eval thing" with one owner and zero process. Should have been a real product with a roadmap from week one.

**Write the deploy gate before the dashboard.** We built the dashboard first because it was more visible and felt like progress. The deploy gate caught more regressions in its first week than the dashboard caught in its first month. Block bad changes from shipping; *then* build the surfaces to investigate them.

**Ship the sandbox earlier.** We built review and dashboards first, sandbox last — the order felt right because review was where we were drowning. In hindsight the sandbox unlocked an order of magnitude more leverage. SMEs going from "rate the AI" to "design the AI" was the moment the iteration loop actually closed. If we did it again, the sandbox would be week three.
