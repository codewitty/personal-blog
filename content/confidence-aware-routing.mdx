...
description: At Aurelian I built confidence-aware routing for a conversational AI that handles non-emergency public-safety calls. The most important code in the system is the part that decides not to use the AI. Here's how I built it — and what false-confident LLM outputs taught me about the limits of model self-assessment.
...
- **Tunable.** Thresholds are config, not weights. I can adjust <Snippet>DISTRESS_THRESHOLD</Snippet> per agency, time of day, or specific incident type without touching the model.
...
- **Plugged into the eval harness.** The policy fires through the same eval framework I wrote about [in the previous post](/blog/prompt-observability-console). I can run the entire policy against thousands of canonical calls in CI and catch regressions before deploy.
